"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var place_1 = require('./place');
var router_1 = require('@angular/router');
var common_1 = require('@angular/common');
var place_service_1 = require('./place.service');
require('rxjs/add/operator/switchMap');
var PlaceDetailComponent = (function () {
    function PlaceDetailComponent(placeService, route, location) {
        this.placeService = placeService;
        this.route = route;
        this.location = location;
    }
    PlaceDetailComponent.prototype.ngOnInit = function () {
        var _this = this;
        this.route.params
            .switchMap(function (params) { return _this.placeService.getPlace(+params['id']); })
            .subscribe(function (place) { return _this.place = place; });
    };
    PlaceDetailComponent.prototype.goBack = function () {
        this.location.back(); //Goes back to last Page in browser history
    };
    __decorate([
        core_1.Input(), 
        __metadata('design:type', place_1.Place)
    ], PlaceDetailComponent.prototype, "place", void 0);
    PlaceDetailComponent = __decorate([
        core_1.Component({
            moduleId: module.id,
            selector: 'my-place-detail',
            templateUrl: 'place-detail.component.html'
        }), 
        __metadata('design:paramtypes', [place_service_1.PlaceService, router_1.ActivatedRoute, common_1.Location])
    ], PlaceDetailComponent);
    return PlaceDetailComponent;
}());
exports.PlaceDetailComponent = PlaceDetailComponent;
//# sourceMappingURL=place-detail.component.js.map